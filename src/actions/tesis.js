...
export const startNewTesisGrado = (formValues) => {
  return async (dispatch) => {
    ...
    for (let i = 0; i < formValues.alumnosLista.length; i++) {
      const docRef = doc(
        db,
        'Niveles/Grado/Licenciatura',
        formValues.alumnosLista[i],
      );
      try {
    ...
    if (alumnosArray.length == 0) {
      if (Data.docs.length == 0) {
        ...
        Swal.fire(
          'Error',
          'La tesis que deseas agregar ya está registrada',
          'error',
        );
      }
    } else {
      Swal.fire(
        'Error',
        'Cada alumno solo puede tener una tesis por grado. Los alumnos que no cumplen con esto son: ' +
          alumnosArray,
        'error',
      );
    }
...
export const startNewTesisPosgrado = (formValues) => {
  return async (dispatch) => {
    ...
    if (alumno != 'exist') {
      ...
      Swal.fire(
        'Error',
        'Cada alumno solo puede tener una tesis por grado',
        'error',
      );
    }
  };
