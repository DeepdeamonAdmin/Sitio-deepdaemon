...
export const ProjectScreen = ({ status1, status2 }) => {
  ...
  var projects_type = projects.projects.filter(
    (projects) => projects.estado === status1 || projects.estado === status2,
  );
  ...
  return (
    <>
      <div
        className="container card-columns"
        style={{
          display: 'flex',
          flexWrap: 'wrap',
          justifyContent: 'left',
      ...
      >
        {projects_type.length === 0 && (
          <p className="team_title">
            {' '}
            No se encontraron proyectos por el momento.
          </p>
        )}
        {projects_type.map(
          (project) =>
            project.display === 'Yes' && (
              <div key={project.id} className="card-row">
                <div
                  className="row mb-4 bg-light ml-2"
                  style={{
                    maxWidth: '500px',
                    minWidth: '200px',
                    height: '130px',
                ...
                >
                  <div
                    className="col-sm-6 mt-2 ml-2"
                    style={{ margin: '0px', padding: 0, width: '110px' }}
                  >
                    <img
                      className="card-img"
                      src={project.urlImg}
